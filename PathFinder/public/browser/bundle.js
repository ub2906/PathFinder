    ...
    if (!this.currentAlgorithm) {
      ...
      if (this.currentAlgorithm === "bidirectional") {
        if (!this.numberOfObjects) {
          success = bidirectional(this.nodes, this.start, this.target, this.nodesToAnimate, this.boardArray, this.currentAlgorithm, this.currentHeuristic);
    ...
    document.getElementById("startButtonAddEnd").onclick = () => {
      ...
      if (this.currentAlgorithm !== "bidirectional") {
        if (innerHTML.includes("Add")) {
          ...
          let c = Math.floor(2 * this.width/4);
          let endNodeId = `${r}-${c}`;
          if (this.target === endNodeId || this.start === endNodeId) {
            console.log("Failure to place object.");
          } else {
